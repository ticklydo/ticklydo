    ...
    <div style={{ background: "var(--sk-surface)", border: "1px solid var(--sk-border)", borderRadius: 16, padding: "18px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
          <Skeleton w="80%" h={10} /><Skeleton w="60%" h={10} /><Skeleton w="70%" h={10} /><Skeleton w="50%" h={10} />
        </div>
      </div>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "24px 1fr auto auto auto", alignItems: "center", gap: 12, padding: "12px 16px", borderBottom: "1px solid var(--sk-border)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        <Skeleton w="55%" h={12} /><Skeleton w="30%" h={9} />
      </div>
      <Skeleton w={50} h={20} /><Skeleton w={50} h={20} /><Skeleton w={70} h={24} radius={6} />
    </div>
  );
}
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      ...
      {arcs.map((arc, i) => arc.value > 0 && (
        <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={arc.color} strokeWidth={stroke} strokeDasharray={arc.dasharray} strokeDashoffset={0} transform={`rotate(${arc.rotate} ${cx} ${cy})`} strokeLinecap="round" />
      ))}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6, width: "100%" }}>
      {data.map((d, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ flex: 1, height: 20, background: darkMode ? "#ffffff0a" : "#f3f4f6", borderRadius: 6, overflow: "hidden" }}>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
      {days.map((day, i) => {
        ...
        const isToday = i === 0;
        return (
          <div key={i} style={{ borderRadius: 10, padding: "8px 4px", textAlign: "center", background: isToday ? appliedA : (darkMode ? "#ffffff08" : "#f8f9fb"), border: `1px solid ${isToday ? appliedA : theme.border}` }}>
...
const gradientText = (grad: string) => ({ backgroundImage: grad, WebkitBackgroundClip: "text" as const, WebkitTextFillColor: "transparent" as const, backgroundClip: "text" as const });
...
function Heatmap({ data, appliedA, darkMode, theme }: { data: Record<string, number>; appliedA: string; darkMode: boolean; theme: any }) {
  ...
  cols.forEach((col, ci) => {
    ...
    if (ci === 0 || new Date(cols[ci-1][0].date).getMonth() !== d.getMonth()) monthLabels.push({ label: MONTHS[d.getMonth()], col: ci });
  });
  return (
    <div style={{ overflowX: "auto" }}>
      <div style={{ position: "relative", paddingTop: 18, minWidth: cols.length * 13 }}>
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 16 }}>
          {monthLabels.map((m, i) => <span key={i} style={{ position: "absolute", left: m.col * 13, fontSize: 9, color: theme.muted, fontWeight: 700 }}>{m.label}</span>)}
        </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 6, justifyContent: "flex-end" }}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((v, i) => <div key={i} style={{ width: 10, height: 10, borderRadius: 2, background: v === 0 ? (darkMode ? "#ffffff0a" : "#f0f0f0") : appliedA + Math.round(v * 255).toString(16).padStart(2,"0") }} />)}
...
function TrendChart({ tasks, appliedA, darkMode, theme }: { tasks: any[]; appliedA: string; darkMode: boolean; theme: any }) {
  ...
  for (let w = 7; w >= 0; w--) {
    ...
    weeks.push({ label: `${weekStart.getDate()}.${weekStart.getMonth()+1}`, total: weekTasks.length, done: weekTasks.filter(t => t.status === "Hotovo").length });
  }
      ...
      {weeks.map((w, i) => (
        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
          <div style={{ width: "100%", display: "flex", flexDirection: "column", justifyContent: "flex-end", height: chartH, gap: 2 }}>
            {w.total > 0 ? (
              <div style={{ width: "100%", height: Math.max((w.total / maxVal) * chartH, 4), background: darkMode ? appliedA + "33" : appliedA + "22", borderRadius: "4px 4px 0 0", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
                <div style={{ width: "100%", height: Math.max((w.done / maxVal) * chartH, w.done > 0 ? 4 : 0), background: appliedA, borderRadius: "0", transition: "height .4s ease" }} />
              </div>
            ) : <div style={{ width: "100%", height: 3, background: darkMode ? "#ffffff0a" : "#f0f0f0", borderRadius: 2 }} />}
          </div>
...
function BurndownChart({ tasks, appliedA, darkMode, theme }: { tasks: any[]; appliedA: string; darkMode: boolean; theme: any }) {
  ...
  const totalTasks = tasks.filter(t => { if (!t.dueDate) return false; const d = new Date(t.dueDate); return d.getMonth() === today.getMonth() && d.getFullYear() === today.getFullYear(); }).length;
  if (totalTasks === 0) return <div style={{ textAlign: "center", padding: "20px 0", color: theme.muted, fontSize: 12 }}>Žiadne úlohy s termínom tento mesiac</div>;
  ...
  const idealLine = Array.from({ length: daysInMonth }, (_, i) => ({ day: i + 1, ideal: Math.round(totalTasks * (1 - (i + 1) / daysInMonth)) }));
  const chartH = 80; const chartW = 100;
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${chartW} ${chartH}`} style={{ width: "100%", height: chartH * 2, overflow: "visible" }}>
        ...
        {points.length > 0 && <circle cx={toX(points[points.length-1].day)} cy={toY(points[points.length-1].remaining)} r="2.5" fill={appliedA} />}
      </svg>
...
export default function WorkPage() {
  ...
  const updateTaskStatus = async (task: Task, newStatus: Status) => {
    ...
    const projectPath = task.projectId.startsWith("shared_")
      ? task.projectId.replace("shared_", "").replace(/_([^_]+)$/, "_$1")
      : `${user.uid}_${task.projectId}`;
    ...
    await setDoc(doc(db, "projects", projectPath), { tasks: updatedTasks }, { merge: true });
  ...
  if (loading) return (
    <div style={{ flex: 1, overflowY: "auto", padding: isMobile ? "16px 12px 60px" : "24px 24px 60px", display: "flex", flexDirection: "column", gap: 20, background: theme.bg, fontFamily: "var(--font-geist-sans)" }}>
      <style>{`@keyframes skeletonShimmer{0%{background-position:200% 0}100%{background-position:-200% 0}} :root{--sk-a:${darkMode?"#ffffff0a":"#f0f0f0"};--sk-b:${darkMode?"#ffffff16":"#e0e0e0"};--sk-surface:${darkMode?theme.card:"#ffffff"};--sk-border:${theme.border};}`}</style>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}><Skeleton w={22} h={22} radius={4} /><Skeleton w={160} h={28} radius={8} /></div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14 }}><SkeletonCard /><SkeletonCard /><SkeletonCard /></div>
      <div style={{ background: darkMode ? theme.card : "#fff", border: `1px solid ${theme.border}`, borderRadius: 14, padding: "12px 16px", display: "flex", gap: 10 }}><Skeleton w="40%" h={32} radius={8} /><Skeleton w={120} h={32} radius={8} /><Skeleton w={120} h={32} radius={8} /></div>
      <div style={{ background: darkMode ? theme.card : "#fff", border: `1px solid ${theme.border}`, borderRadius: 14, overflow: "hidden" }}>{[...Array(6)].map((_, i) => <SkeletonRow key={i} />)}</div>
    </div>
  );
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: isMobile ? "16px 12px 60px" : "24px 24px 60px", display: "flex", flexDirection: "column", gap: 20, background: theme.bg, color: theme.text, fontFamily: "var(--font-geist-sans)", transition: "background .3s, color .3s" }}>
      <style>{`@keyframes fadeIn{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:translateY(0)}} @keyframes skeletonShimmer{0%{background-position:200% 0}100%{background-position:-200% 0}} select{appearance:none!important;-webkit-appearance:none!important} :root{--sk-a:${darkMode?"#ffffff0a":"#f0f0f0"};--sk-b:${darkMode?"#ffffff16":"#e0e0e0"};--sk-surface:${darkMode?theme.card:"#ffffff"};--sk-border:${theme.border};}`}</style>
          ...
          <div style={{ background: surface, border: `1px solid ${theme.border}`, borderRadius: 16, padding: "18px 20px", boxShadow: shadow, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <div style={{ flex: 1, background: "#fee2e2", borderRadius: 8, padding: "8px", textAlign: "center" }}>
                <div style={{ fontSize: 18, fontWeight: 900, color: "#dc2626" }}><AnimatedNumber target={totalOverdue} color="#dc2626" /></div>
